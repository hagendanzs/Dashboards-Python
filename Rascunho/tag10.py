...
class MonitorCLPApp:
    ...
    def atualizar_dados(self):
        ...
        # Atualiza o LED de Status de Comunicação Geral
        ...
        # Atualização da Tabela de Variáveis Dinâmicas
        for resposta in (retorno if retorno else []):
            ...
            if tag_nome_low in self.widgets_dados:
                ...
                if resposta.Status == "Success":
                    ...
                    widgets["status_widget"].config(text="Success", fg="#00ff66")
                    ...
                    widgets["status_widget"].config(text="Erro", fg="#ff0000")

        # Caso a comunicação falhe por completo
        if not sucesso_comunicacao:
            for tag_low, widgets in self.widgets_dados.items():
                widgets["value_widget"].config(text="---")
                widgets["status_widget"].config(text="Offline", fg="#ff0000")
