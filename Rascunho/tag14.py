...
class MonitorCLPApp:
    ...
    def atualizar_dados(self):
        ...
        try:
            ...
            # Validação se o retorno é uma lista válida de objetos de resposta do pylogix
            ...
            for resposta in retorno:
                if hasattr(resposta, 'TagName') and resposta.TagName.lower() in self.widgets_dados:
                    ...
                    if resposta.Status == "Success":
                        ...
                        widgets["canvas_widget"].itemconfig(widgets["led_id"], fill="#00ff66")  # Verde Claro
                        ...
                        widgets["canvas_widget"].itemconfig(widgets["led_id"], fill="#004d1a")  # Verde Escuro

        else:
            # --- TRATAMENTO SEGURO DE FALHA DE LEITURA (OFFLINE/CONEXÃO PERDIDA) ---
            self.canvas_led.itemconfig(self.led_id, fill="#ff0000")  # LED Principal Vermelho
            self.lbl_relogio_hora.config(text="--:--:--")
            self.lbl_relogio_data.config(text="DATA: --/--/----")

            # Aplica o reset visual de erro (Verde Escuro) em todos os registros da tabela imediatamente
            for tag_low, widgets in self.widgets_dados.items():
                widgets["value_widget"].config(text="---")
                widgets["canvas_widget"].itemconfig(widgets["led_id"], fill="#004d1a")  # Verde Escuro (Fail)

        # Atualiza a tela gráfica mesmo sem comunicação (mostra o histórico congelado ou estático)
        self.desenhar_grafico()
        
        # Mantém o loop ativo infinitamente, permitindo a autoreconexão quando o cabo for plugado
