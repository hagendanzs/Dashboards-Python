...
class MonitorCLPApp:
    ...
    def criar_layout(self):
        ...
        # ----------------------------------------------------------------------
        # Atualizado o título do frame para indicar as duas penas do gráfico
    ...
    def atualizar_dados(self):
        ...
        try:
            ...
            if retornos and comunicacao_ok:
                ...
                hora = dados.get('relogio[3]', 0)
                minuto = dados.get('relogio[4]', 0)
                segundo = dados.get('relogio[5]', 0)
                dia = dados.get('relogio[2]', 0)
                mes = dados.get('relogio[1]', 0)
                ano = dados.get('relogio[0]', 0)
                ...
                # Alimenta o histórico da pena de segundos (escala de 0 a 59)
                ...
                for tag_mapeada, widgets in self.widgets_dados.items():
                    if tag_mapeada in dados:
                        ...
                        # Alimenta o histórico da pena Conta[0].acc
                        if tag_mapeada == 'conta[0].acc':
    ...
    def desenhar_grafico(self):
        ...
        # Gerencia o tamanho dos arrays para manter a taxa de pontos síncrona
        ...
        # ----------------------------------------------------------------------
        # PENA 1: Conta[0].acc (Cor: Laranja)
        # ----------------------------------------------------------------------
        ...
        if len(self.historico_segundos) >= 2:
            # Como segundos variam estritamente de 0 a 59, usamos limites fixos para estabilizar a onda
            min_s, max_s = 0, 59
            range_s = max_s - min_s
            ...
            for i in range(len(pontos_seg) - 1):
                ...
                # Linha tracejada (dash) em ciano para destacar a diferença visualmente
