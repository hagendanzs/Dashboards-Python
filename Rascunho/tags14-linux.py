...
# ==============================================================================
# FUNÇÃO PARA CARREGAR FONTE EM TEMPO DE EXECUÇÃO (MULTIPLATAFORMA)
# ==============================================================================
def carregar_fonte_local(caminho_fonte):
    """Carrega temporariamente um arquivo .ttf na memória do SO para o script usar."""
    ...
    if sys.platform == "win32":
        ...
        if retorno != 0:
            print(f"Font carregada com sucesso no Windows a partir de: {caminho_fonte}")
            return True
        else:
        ...
        # No Linux, criamos a pasta de fontes local do usuário caso não exista e linkamos a fonte
        pasta_fontes_usuario = os.path.expanduser("~/.local/share/fonts")
        try:
            os.makedirs(pasta_fontes_usuario, exist_ok=True)
            destino_fonte = os.path.join(pasta_fontes_usuario, os.path.basename(caminho_fonte))
            
            # Se a fonte ainda não está lá, copia ou cria link simbólico
            if not os.path.exists(destino_fonte):
                import shutil
                shutil.copy(caminho_fonte, destino_fonte)
                # Atualiza o cache de fontes do Linux de forma assíncrona
                os.system("fc-cache -f")
            
            print(f"Font configurada com sucesso no Linux em: {destino_fonte}")
            return True
        except Exception as e:
            print(f"Não foi possível carregar a fonte no Linux: {e}")
            return False
            
    else:
        print(f"Carregamento dinâmico automático não implementado para o sistema: {sys.platform}")
        return False
...
class MonitorCLPApp:
    def __init__(self, root):
        ...
        self.fonte_lcd_grande = tkfont.Font(family=NOME_INTERNO_FONTE, size=44, weight="bold")
        self.fonte_lcd_media = tkfont.Font(family=NOME_INTERNO_FONTE, size=18, weight="bold")
    ...
    def criar_layout(self):
        ...
        lbl_led_texto = tk.Label(linha2_frame, text="Status da comunicação:", font=self.fonte_cabecalho, fg="#ffffff", bg="#1e1e1e")
        ...
        # --- BOTÃO PARA ZERAR CONTADORES ---
        btn_frame = tk.Frame(self.root, bg="#1e1e1e")
        btn_frame.pack(padx=20, pady=5, fill=tk.X)
        
        self.btn_zerar = tk.Button(
            btn_frame, 
            text="ZERAR CONTADORES (CONTA[0] E CONTA[1])", 
            font=self.fonte_cabecalho, 
            fg="#1e1e1e", bg="#ff9900", 
            activebackground="#cc7a00", activeforeground="#1e1e1e",
            bd=0, height=1, cursor="hand2",
            command=self.zerar_contadores
        )
        self.btn_zerar.pack(fill=tk.X, ipady=4)
        ...
        frame_grafico = tk.LabelFrame(
            self.root, 
            text=" TENDÊNCIA EM TEMPO REAL ( Laranja Pontilhado: Conta[0].acc | Ciano: Segundos ) ", 
            font=self.fonte_cabecalho, fg="#ffffff", bg="#1e1e1e", bd=1, relief=tk.SOLID
        )
    ...
    def zerar_contadores(self):
        """Escreve o valor zero nas variáveis Conta[0].acc e Conta[1].acc via pylogix."""
        try:
            self.btn_zerar.config(state=tk.DISABLED, bg="#555555")
            self.root.update_idletasks()
            res0 = self.comm.Write('Conta[0].acc', 0)
            res1 = self.comm.Write('Conta[1].acc', 0)
            
            if res0 and res1 and res0.Status == "Success" and res1.Status == "Success":
                self.lbl_ip.config(text=f"CLP IP: {IP_CLP} - CONTADORES ZERADOS OK!", fg="#00ff66")
                self.historico_conta0.clear()
            else:
                self.lbl_ip.config(text=f"CLP IP: {IP_CLP} - FALHA AO ZERAR (PROIBIDO PELO CLP)", fg="#ff0000")
        except Exception:
            self.lbl_ip.config(text=f"CLP IP: {IP_CLP} - ERRO DE ESCRITA", fg="#ff0000")
        finally:
            self.root.after(1500, lambda: [
                self.btn_zerar.config(state=tk.NORMAL, bg="#ff9900"),
                self.lbl_ip.config(text=f"CLP IP: {IP_CLP}", fg="#9cdcfe")
            ])

    def atualizar_dados(self):
        """Efetua a leitura física no CLP via pylogix de forma segura e resiliente."""
        retorno = None
        ...
        try:
            ...
            if retorno and isinstance(retorno, list) and hasattr(retorno[0], 'Status'):
        ...
        # --- PROCESSAMENTO DO STATUS DA COMUNICAÇÃO ---
        if sucesso_comunicacao:
            ...
            # Atualização do Bloco de Horário
            try:
            ...
            # Atualização individual das Tags válidas
            for resposta in retorno:
                if hasattr(resposta, 'TagName') and resposta.TagName.lower() in self.widgets_dados:
            ...
            # --- TRATAMENTO SEGURO DE FALHA DE LEITURA (OFFLINE/CONEXÃO PERDIDA) ---
            self.canvas_led.itemconfig(self.led_id, fill="#ff0000")
    ...
    def desenhar_grafico(self):
        ...
        plotar_pena(self.historico_conta0, "#ff9900", max_escala=60, pontilhada=True)
